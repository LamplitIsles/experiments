...
test("automatic name is persisted through the official app-server protocol", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "zencodex-title-"));
  let server: Awaited<ReturnType<typeof connect>> | undefined;
  try {
    ...
    server.onNotification("thread/name/updated", (params) => {
      if (params.threadId === thread.id) updated.resolve(params.threadName);
    });
    await server.nameThreadFromPrompt(
      thread.id,
      "Fix the login timeout",
      "fixture-model",
    );
    expect(await updated.promise).toBe("Fix login timeout");
    const state = JSON.parse(
      await readFile(join(cwd, ".fake-app-server-state.json"), "utf8"),
    );
    expect(state.threadName).toBe("Fix login timeout");
    ...
    expect(
      calls.find(
        (call) => call.method === "thread/start" && call.params.ephemeral,
      )?.params.model,
    ).toBe("fixture-model");
  } finally {
    await server?.close();
